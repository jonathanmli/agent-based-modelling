...
class SimilarElement_g(TextElement):
    ...
    def render(self, model):
        return "Groups avg. % similar neighbors: (A) " + str(model.pct_neighbors0) + "%" + " (B) " + str(
            model.pct_neighbors1) + "%" + " (C) " + str(model.pct_neighbors2) + "%"
...
happy_chart2 = ChartModule([{"Label": "Pct Happy Group C", "Color": "Blue"}])
...
model_params = {
    ...
    "minority1_pc": UserSettableParameter('slider', "% group B", 0.2, 0.00, 1.0, 0.05),
    "minority2_pc": UserSettableParameter('slider', "% group C", 0.1, 0.00, 1.0, 0.05),
...
server = ModularServer(
    ...
    [canvas_element, happy_element,
     ...
     happy_chart, happy_chart1, happy_chart2],
