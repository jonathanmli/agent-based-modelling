...
from wolf_sheep_original.scheduler import RandomActivationByTypeFiltered
from wolf_sheep_original.agents import Sheep, Wolf, GrassPatch
...
class WolfSheep(mesa.Model):
    ...
    def step(self):
        if self.step_by_type:
            # step type by type. This ensures that types with more population do not move disproportionally 
            # the order is grass, sheep, and wolf
            self.schedule.step(shuffle_types = False)
            #self.schedule.step_type(GrassPatch)
            #self.schedule.step_type(Sheep)
            #self.schedule.step_type(Wolf)
        else:
            self.schedule.step(shuffle_types = True)
