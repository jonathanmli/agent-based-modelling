...
class SegModel(Model):
    ...
    def __init__(self, width, height, num_agents, minority1_pc, minority2_pc, intolerance):
        self.num_agents = num_agents  # we're allowing these values to be set at each run
        self.minority1_pc = minority1_pc
        self.minority2_pc = minority2_pc
        ...
        self.pct_neighbors2 = 0
        self.pct_neighbors_e = 0
        self.pct_neighbors_e0 = 0
        self.pct_neighbors_e1 = 0
        self.pct_neighbors_e2 = 0

        # placing agents at random in the world
        # setting finite number of each agent type
        self.num_agents2 = round(self.num_agents * (self.minority2_pc))
        self.num_agents1 = round(self.num_agents * (self.minority1_pc)) 
        self.num_agents0 = self.num_agents - self.num_agents1 - self.num_agents2

        for i in range(self.num_agents):

            if i < self.num_agents2:
                self.agent_type = 2
            elif self.num_agents2 <= i < self.num_agents2 + self.num_agents1:
                self.agent_type = 1
            else:
                self.agent_type = 0
        ...
        self.datacollector = DataCollector(
            model_reporters={"Pct Happy": lambda m: round(100 * m.happy / m.num_agents, 1),
                             ...
                             "Pct Happy Group C": lambda m: round(100 * m.happy2 / m.num_agents2, 1),
                             ...
                             "Avg pct similar neighbors (C)": lambda m: m.pct_neighbors2,
                             ...
                             "Avg pct similar neighbors (C) (count empty)": lambda m: m.pct_neighbors_e2,
                             "Num Agents": lambda m: m.num_agents,
                             "Num Agents (A)": lambda m: m.num_agents0,
                             "Num Agents (B)": lambda m: m.num_agents1,
                             "Num Agents (C)": lambda m: m.num_agents2,
                             "Pct group B": lambda m: m.minority1_pc,
                             "Pct group C": lambda m: m.minority2_pc, 
    ...
    def step(self):
        ...
        self.happy2 = 0  # Reset counter of happy agents
        ...
        self.similar_g2 = 0  # Reset counter of similar agents
        self.neighbors_g = 0
        self.neighbors_g0 = 0
        self.neighbors_g1 = 0
        self.neighbors_g2 = 0
        ...
        for agent in self.schedule.agents:
            ...
            if agent.type == 0:
                ...
                self.neighbors_g2 += agent.neighbors_a
                self.similar_g2 += agent.similar2
        ...
        self.pct_neighbors_e2 = round(100 * self.similar_g2 / (8 * self.num_agents2), 1)
        ...
        if self.neighbors_g == 0:
            ...
            self.pct_neighbors2 = round(100 * self.similar_g2 / self.neighbors_g2, 1)
